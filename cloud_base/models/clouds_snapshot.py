...
FOLDER_ATTRS = ["client_id", "active", "name", "parent_id", "cloud_key", "parent_reassigned"]
...
class clouds_snapshot(models.Model):
    ...
    folder_id = fields.Many2one("clouds.folder", "Folder", ondelete="cascade", index=True)
    snapshot = fields.Text("Snapshot", default="{}")
    back_snapshot = fields.Text("Cloud Snapshot", default="{}")
    last_sync_datetime = fields.Datetime("Queue Time", default=lambda self: fields.Datetime.now(), index=True)
    block_sync_datetime = fields.Datetime("Block Time", index=True)
    parent_id = fields.Many2one("clouds.snapshot", compute=_compute_parent_id, compute_sudo=True, store=True)
    task_ids = fields.One2many("clouds.queue", "snapshot_id", string="Tasks")
    reverse_state = fields.Selection(
        [("normal", "Normal"), ("rearranged", "Reversed"), ("await", "Await Reversed")],
        default="normal",
        string="Reverse State",
        index=True,
    )
    move_state = fields.Selection(
        [("normal", "Normal"), ("special_blocking", "Sync blocked")],
        default="normal",
        string="Move State",
        index=True,
    ...
    def _prepare_folder_queue(self):
        ...
        Methods:
         * _return_block_datetime of clouds.folder
        ...
        snap_vals = {
            ...
            "block_sync_datetime": self.folder_id._return_block_datetime(),
        }
        ...
        if checked_client.restrict_db_sync and checked_client.restrict_db != self.env.cr.dbname:
            # we check do not checked db-blocked clients
            return snap_vals
        ...
        def _prepare_direct_sync_queue_folders():
            ...
            """
            if "parent_id" in folder_changes or "name" in folder_changes or (
                    "parent_reassigned" in folder_changes and new_state.get("parent_reassigned")):
            ...
            for new_attach_dict in new_attachment_ids:
                ...
                if pre_attach_dict_list:
                    ...
                    if new_attach_dict.get("cloud_key"):
                        ...
                        # IMPORTANT: re-arranged sync_cloud_folder_id is totally fine here
                        ...
                        move_attachments.append(new_attach_id)
                        ...
                        if sync_cloud_folder_id and sync_cloud_folder_id != new_cl_folder_id:
                            if not new_cl_folder_id or not new_cl_folder_id.client_id \
                                ...
                                if attachment_id.cloud_key:
                                    ...
                                    move_attachments.append(pre_attach_id)
        ...
        def _prepare_back_sync_queue():
            ...
            for del_atta_dict in to_delete_attachment_list:
                ...
                if exist_in_odoo and exist_in_odoo[0] not in move_attachments:
            ...
            for new_attach_dict in new_attachment_ids:
                if new_attach_dict.get("cloud_key"):
                    ...
                    if sync_cloud_folder_id and sync_cloud_folder_id != self.folder_id:
                        ...
                        move_attachments.append(new_attach_dict.get("id"))
                        self._block_snapshot_for_move(sync_cloud_folder_id)
                        # if folder is re-arranged, it would be marked so afterwards. It cannot be re-arranged before
                        snap_vals.update({"move_state": "special_blocking"})
                else:
        ...
        def _reverse_sync(subfolder_vals):
                ...
                if task.active:
                    if task.task_type == "reverse":
                        ...
                        if task.name in ["create_subfolder", "create_attachment"]:
                            ...
                            # IMPORTANT: the method assumes the first layer of the folders but in the related task
                            # the method _adapt_for_reverse_creation is triggred and in this way the whole
                            # hierarchy will be reflected
                            task.write({
                                "name": "{}_reverse".format(task.name),
            ...
            child_snapshot_ids = self.env["clouds.snapshot"].search([
                ("reverse_state", "=", "normal"), ("id", "child_of", self.id), ("id", "!=", self.id),
            ])
        ...
        if "client_id" in folder_changes:
            ...
            if pre_client_id:
                # old client is unlinked from a folder > get back everything from client to Odoo
                # it also covers the case when a client was deactivated (in resetting, client is unlinked from folders)
                # finally, it covers just the case when a folder gets a new client
                _reverse_sync({
    ...
    def _get_current_odoos_state(self, folder_id):
        ...
        folder_vals.update({"attachment_ids": attachment_vals_list, "child_ids": child_folder_vals_list})
        return folder_vals
    ...
    def _extend_based_on_direct_sync(self, folder_id, attachment_id, action="create"):
        """
        The method to change odoo previous state based on the results of direct sync
        ...
        if action == "create":
            ...
            existing_attachment = [
                attach_dict for attach_dict in attachment_vals_list
