...
class KsBaseGanttExportBase(ExcelExport):
    ...
    def ks_gantt_base_export_excel(self, ks_fields, ks_model_name, ks_domain, ks_context):
        ...
        ks_fields_info = self.ks_get_field_info(ks_fields,ks_model_name)
        response_data = self.from_data(ks_fields_info, ks_fields, ks_export_data)
    ...
    def ks_get_field_info(self,ks_fields,ks_model_name):
        """
            Function to return more field information of exported fields.
            :return:
        """
        model = request.env[ks_model_name]
        ks_fields_info = []
        for ks_export_field in ks_fields:
            field = model._fields[ks_export_field]
            ks_fields_info.append({'name': field.name,'label': field.string,'store': field.store,'type': field.type})
        return ks_fields_info
