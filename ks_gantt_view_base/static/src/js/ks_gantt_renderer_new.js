...
export class ksGanttRenderer extends Component{
    static props = [
        "model",
        "arch",
        "class",
    ...
    setup(){
       ...
       var params = this.props.model.metaData;
      this.props.model.ksDataForGantt.records.forEach(record => {
       record.ks_tip = false;
       });
      this.ks_gantt_rendered = false;
      this.ks_gantt_config = params.info.ks_gantt_config;
      ...
      this.ks_fieldDetail = params.fields;
      this.string = params.string;
      this.ks_model_name = params.resModel;
      gantt.ks_model_name = params.resModel;
      this.active_id = this.props.model.context.active_id;
      ...
      this.ks_export_field = params.info.ks_export_field;
      ...
      onWillUnmount(()=> this.ks_unmount_gantt());


      // TODO (remove external listener because this is not the correct way to do it.)
      useExternalListener(document, "click", this.clickGridButton.bind(this));


    }
    willstart(){
        var self = this;
      var ksGetTheme = rpc("/web/dataset/call_kw/res.config.settings/ks_gantt_view_theme",{
    ...
    ks_renderGantt () {
      ...
      this.props.ksGanttConfig();
      this.props.ksGanttCreateDp();
      ...
      gantt.config.ks_attached_events.push(
        gantt.attachEvent("onTaskLoading", function (task) {
          if (task.deadline) task.deadline = task.deadline
          return true
        })
      );
      ...
      gantt.init($(".ks_gantt_view_content").get(0));
...
//      if (gantt.config.ks_render_count) {
        gantt.clearAll();
        gantt.config.ks_disable_graph_overlay = false;
//      }
//      gantt.config.ks_render_count = true;
    ...
    ks_left_grid_columns() {
    var self = this;
      var ks_col_header = renderToString("ks_gantt_view_base.ks_col_header"),
        ks_col_content_buttons = function (task) {
           return renderToString("ks_gantt_view_base.ks_col_content_buttons", {
            task: task,
            create:
      ...
      if (this.ks_gantt_config && !this.ks_gantt_config.ks_hide_grid_owner) {
        ks_gantt_config_columns.push({
          ...
          template: function (task) {
            if (task.ks_owner_task) {
              if (Array.isArray(task.ks_owner_task)) return task.ks_owner_task[1];
       ...
       if (this.ks_gantt_arch && this.ks_gantt_arch.project_id) {
          ks_gantt_config_columns.push({
            name: "project_id",
            width: 150,
            label: _t("Project"),
      ...
      if (this.ks_gantt_arch && this.ks_gantt_arch.planned_hours) {
          ks_gantt_config_columns.push({
            name: "planned_hours",
            width: 100,
            label: _t("Planned Hours"),
            align: "center",
            sort: false,
            template: function (task) {
...
//       Added grid button.
    ...
    clickGridButton(ev) {
        if ($(ev.target).hasClass("ks_gantt_grid_edit")){
             const task_id = ev.target.getAttribute("task_id");
             this.props.openDialog({resId:task_id})
        }else if ($(ev.target).hasClass("ks_gantt_grid_delete")){
            const task_id = ev.target.getAttribute("task_id");
            this.props._KsOnDeleteRecord(task_id)
        }else if ($(ev.target).hasClass("ks_gantt_grid_add")){
            var context = {};
            var task_id = ev.target.getAttribute("task_id");
            if (task_id) {
    ...
    ks_task_drag_and_drop () {
      ...
      if (
        this.props.model.ksDataForGantt.groupedBy.length == 0 &&
        this.ks_model_name != "project.project"
      ) {
    ...
    ks_handle_non_working_timeline() {
...
//          els.appendChild(task_duration_el);
      ...
      for (var i = 0; i < gantt.config.ks_exclude_holiday.length; i++) {
        gantt.setWorkTime({
          date: this.ksConversionUtcToLocal(
            parseDateTime(gantt.config.ks_exclude_holiday[i],localization.dateTimeFormat)
          ),
          hours: false,
        });
      }
        ...
        for (var i = 0; i < tasks.length; i++) {
          ...
          while (currDate < task.end_date) {
            ...
            if (task.end_date > currDate && task.start_date < currDate)
              ks_end_hours = new Date(currDate);
              if(task.ks_resource_hours_available.is_flexible){
                if (step == 'minute' || step == 'hour') {
                    timegrid[timestamp] = 0;
                }else{
                    while (ks_start_hours < ks_end_hours) {
                     timegrid[timestamp] += task.ks_resource_hours_available.work_hours;
                     ks_start_hours.setHours(ks_start_hours.getHours() + 24);
                   }
                }
              }else{
                while (ks_start_hours < ks_end_hours) {
    ...
    ks_parse_gantt_data() {
      ...
      if (!this.props.model.ksDataForGantt.groupedBy.length) {
        this.props.model.ksDataForGantt.records.forEach(
          function (each_task) {
            this.ks_data_update(
              each_task,
              ks_links,
        ...
        ks_data.sort((a, b) => a.sequence - b.sequence);
        }
   ...
   ks_render_data_group_by () {
      ...
      var ks_id_list = [];
      var ks_gantt_fields = this.ks_gantt_arch;
      var ks_group_name = this.props.model.ksDataForGantt.groupedBy[0].split(':')[0]
      var ks_groupb_date = this.props.model.ksDataForGantt.groupedBy.length-1

      var ks_new_group_name = this.props.model.ksDataForGantt.groupedBy[ks_groupb_date].split(':')[0]
      if (this.props.model.ksDataForGantt.ks_group_by_records) {
        for (var group_index in this.props.model.ksDataForGantt.ks_group_by_records) {
          var parent_group_id = 0;
          var ks_group_by_dict = {};
          var each_group = this.props.model.ksDataForGantt.ks_group_by_records[group_index];
          // Multi level group-by
          // This will create a hierarchy for multi level group-by and have parent id for data.
          for (var group_lvl in this.props.model.ksDataForGantt.groupedBy) {

            if (this.ks_fieldDetail[ks_new_group_name].type == 'datetime') {
                var ks_date_con = this.props.model.ksDataForGantt.groupedBy[group_lvl].split(':')[0]
                var ks_project_id = this.props.model.ksDataForGantt.groupedBy[group_lvl]
                var vars = (Object.keys(each_group.__range)[0]).split(":")[0]
                var varz= Object.keys(each_group.__range)[0]

                if (this.props.model.ksDataForGantt.groupedBy.length > 1){
                    var ks_groupb_date = this.props.model.ksDataForGantt.groupedBy.length-2

                    var ks_new_project_id = this.props.model.ksDataForGantt.groupedBy[ks_groupb_date].split(':')[0]
                    if (this.ks_fieldDetail[ks_new_project_id].type == 'datetime') {
                    ks_new_project_id = ks_new_project_id.split(' ')[0]
                    }
                }
                else {
                var ks_groupb_date = this.props.model.ksDataForGantt.groupedBy.length-1
...
//                var ks_new_project_id = this.props.model.ksDataForGantt.groupedBy[ks_groupb_date].split(':')[0]
                var sub_group_name = ks_date_con;

                var ks_each_group_sub_group = this.props.model.ksDataForGantt.groupedBy[group_lvl];
            }
            if (this.ks_fieldDetail[ks_new_group_name].type != 'datetime') {
                var ks_project_id = this.props.model.ksDataForGantt.groupedBy[0].split(':')[0]
                var sub_group_name = this.props.model.ksDataForGantt.groupedBy[group_lvl].split(':')[0];
                var ks_each_group_sub_group = this.props.model.ksDataForGantt.groupedBy[group_lvl];
            ...
            if (parseInt(group_lvl) < 1) {
              ...
              if (sub_group) {
                ks_data.push({
                  ...
                  type: "project",
                  open: true,
                  planned_hours: each_group.planned_hours,
                });
                parent_group_id =
                  "group_" +
                  (typeof sub_group == "string" ? sub_group : sub_group[0]);
          ...
          for (var ks_task_index in this.props.model.ksDataForGantt.records) {
            var ks_each_task = this.props.model.ksDataForGantt.records[ks_task_index];
            var ks_render_check = true;
            if (ks_each_task[sub_group_name].length != 0) {

            if (this.ks_fieldDetail[ks_new_group_name].type != 'datetime') {

                if (this.ks_fieldDetail[ks_project_id].type != 'datetime') {

                 if(this.props.model.ksDataForGantt.groupedBy.length > 1) {
                    if ((ks_each_task[sub_group_name] == each_group[sub_group_name]) && (ks_each_task[ks_project_id][0] == each_group[ks_project_id][0])) {
                    ...
                    if (ks_each_task[ks_project_id].length == 2) {
                            if (ks_each_task[ks_project_id][0] == each_group[ks_project_id][0]) {
                                for (var ks_check_group_index in ks_group_by_dict) {
                              var ks_check_group = ks_group_by_dict[ks_check_group_index];
                            ...
                            //                        if (this.props.model.ksDataForGantt.groupedBy[group_lvl]==='user_ids'){
                            // var ks_parent = 0
                            ...
                            if (ks_each_task[ks_project_id] == each_group[ks_project_id]) {
                                for (var ks_check_group_index in ks_group_by_dict) {
                              var ks_check_group = ks_group_by_dict[ks_check_group_index];
                              //                            if (JSON.stringify(ks_check_group) != JSON.stringify(ks_each_task[ks_check_group_index])) {
                              //                                ks_render_check = false;
                ...
                if (each_group.__range[varz] != false)  {

                if (ks_each_task[vars] != false) {
                   if (ks_each_task[ks_new_group_name].length == 2){
                        var ks_date_tf_value = (ks_each_task[vars].split(' ')[0] > each_group.__range[varz].from.split(' ')[0] &&
                                (ks_each_task[vars].split(' ')[0] <= each_group.__range[varz].to.split(' ')[0] ||
                                ks_each_task[vars].split(' ')[0] == each_group.__range[varz].to.split(' ')[0]) &&
                                ks_each_task[ks_new_group_name][0] == each_group[ks_new_group_name][0])
                    }
                   else {
                        var ks_date_tf_value = (ks_each_task[vars].split(' ')[0] > each_group.__range[varz].from.split(' ')[0] &&
                                (ks_each_task[vars].split(' ')[0] <= each_group.__range[varz].to.split(' ')[0] ||
                               ks_each_task[vars].split(' ')[0] == each_group.__range[varz].to.split(' ')[0]) &&
                                ks_each_task[ks_new_group_name] == each_group[ks_new_group_name] && ks_each_task[ks_new_project_id] == each_group[ks_new_project_id])
                ...
                if (ks_each_task[vars] != false) {
                   ...
                   if (ks_each_task[ks_new_group_name].length == 2){
                        ...
                        var ks_date_tf_value = (ks_each_task[vars].split(' ')[0] > each_group.__range[varz].from.split(' ')[0] &&
                                (ks_each_task[vars].split(' ')[0] <= each_group.__range[varz].to.split(' ')[0] ||
                               ks_each_task[vars].split(' ')[0] == each_group.__range[varz].to.split(' ')[0]) &&
                                ks_each_task[ks_new_group_name] == each_group[ks_new_group_name])
                ...
                if ( ks_date_tf_value == true) {
                    ...
                    var new_sub_group_name = this.props.model.ksDataForGantt.groupedBy[group_lvl]

                    // Assign data with to its group.

                    if (this.ks_fieldDetail[ks_group_name].type == 'datetime') {
                        if (
                          this.ks_compare_group_range_data(
                            ks_count,
                          )
                        )
                        {
            ...
            if (ks_each_task[vars] != false) {
             if (each_group.__range[varz] != false) {
                 if (ks_each_task[ks_group_name].length > 1) {
                     if(ks_each_task[ks_new_project_id].length == 2){
                         var ks_date_tf_value = (ks_each_task[vars].split(' ')[0] > each_group.__range[varz].from.split(' ')[0] &&
                            (ks_each_task[vars].split(' ')[0] <= each_group.__range[varz].to.split(' ')[0] ||
                            ks_each_task[vars].split(' ')[0] == each_group.__range[varz].to.split(' ')[0]) &&
                            (ks_each_task[ks_new_project_id][0] == each_group[ks_new_project_id][0]) && (ks_each_task[ks_group_name][0] == each_group[ks_group_name][0]))
                     }
                    else {
                        var ks_date_tf_value = (ks_each_task[vars].split(' ')[0] > each_group.__range[varz].from.split(' ')[0] &&
                                (ks_each_task[vars].split(' ')[0] <= each_group.__range[varz].to.split(' ')[0] ||
                                ks_each_task[vars].split(' ')[0] == each_group.__range[varz].to.split(' ')[0]) &&
                                (ks_each_task[ks_new_project_id] == each_group[ks_new_project_id]) && (ks_each_task[ks_group_name][0] == each_group[ks_group_name][0]))
                    }
                  }
                     ...
                     if(ks_each_task[ks_new_project_id].length == 2){
                       var ks_date_tf_value = (ks_each_task[vars].split(' ')[0] > each_group.__range[varz].from.split(' ')[0] &&
                            (ks_each_task[vars].split(' ')[0] <= each_group.__range[varz].to.split(' ')[0] ||
                            ks_each_task[vars].split(' ')[0] == each_group.__range[varz].to.split(' ')[0]) &&
                            (ks_each_task[ks_new_project_id][0] == each_group[ks_new_project_id][0]) && (ks_each_task[ks_group_name] == each_group[ks_group_name]))
                         }
                     else {
                        var ks_date_tf_value = (ks_each_task[vars].split(' ')[0] > each_group.__range[varz].from.split(' ')[0] &&
                                (ks_each_task[vars].split(' ')[0] <= each_group.__range[varz].to.split(' ')[0] ||
                                ks_each_task[vars].split(' ')[0] == each_group.__range[varz].to.split(' ')[0]) &&
                                (ks_each_task[ks_new_project_id] == each_group[ks_new_project_id]) && (ks_each_task[ks_group_name] == each_group[ks_group_name]))
                     }
                 }
                 if (this.ks_fieldDetail[ks_new_project_id].type == 'datetime') {
                     if(ks_each_task[ks_group_name].length == 2){
                         var ks_date_tf_value = (ks_each_task[vars].split(' ')[0] > each_group.__range[varz].from.split(' ')[0] &&
                            (ks_each_task[vars].split(' ')[0] <= each_group.__range[varz].to.split(' ')[0] ||
                            ks_each_task[vars].split(' ')[0] == each_group.__range[varz].to.split(' ')[0]) &&
                            (ks_each_task[ks_group_name][0] == each_group[ks_group_name][0]))
                     }
                     else {
                        var ks_date_tf_value = (ks_each_task[vars].split(' ')[0] > each_group.__range[varz].from.split(' ')[0] &&
                                (ks_each_task[vars].split(' ')[0] <= each_group.__range[varz].to.split(' ')[0] ||
                                ks_each_task[vars].split(' ')[0] == each_group.__range[varz].to.split(' ')[0]) &&
                                (ks_each_task[ks_group_name] == each_group[ks_group_name]))
                    }
    ...
    ks_compare_group_by_data(ks_applied_grouped, ks_current_record) {
    ...
    ks_compare_group_range_data(ks_count) {
      if (ks_count == 1) {
        return true;
      }
      else {
    ...
    ks_full_screen_element() {
      ...
      gantt.attachEvent(
        "onBeforeExpand",
        function () {
          $(".ks_gantt_view_content").height(window.outerHeight);
          return true;
        }.bind(this)
      );
    ...
    ks_gantt_view_datetime_format () {
      gantt.config.ks_lang_datetime_format = localization.dateTimeFormat;
      gantt.ks_gantt_view_datetime_format = function (ks_input_datetime) {
        return formatDateTime(luxon.DateTime.fromJSDate(ks_input_datetime),{format:localization.dateTimeFormat})
      };
    }

     ks_handle_gantt_view_marker () {
      // Today Marker
      gantt.addMarker({
        start_date: new Date(),
        css: "today",
      ...
      if (this.ks_project_start) {
        ...
        gantt.addMarker({
          start_date: this.ksConversionUtcToLocal(
            parseDateTime(this.ks_project_start,localization.dateTimeFormat)
          ),
          css: "status_line",
          text: _t("Start"),
          title: gantt.ks_gantt_view_datetime_format(
            this.ksConversionUtcToLocal(parseDateTime(this.ks_project_start,localization.dateTimeFormat))
          ),
        });
      }
      if (this.ks_project_end) {
        // Project End Marker.
        gantt.addMarker({
          start_date: this.ksConversionUtcToLocal(parseDateTime(this.ks_project_end,localization.dateTimeFormat)),
          css: "status_line",
          text: _t("End"),
          title: gantt.ks_gantt_view_datetime_format(
            this.ksConversionUtcToLocal(parseDateTime(this.ks_project_end,localization.dateTimeFormat))
          ),
        });
      }
    }
      ...
      var lineOverlay = overlayControl.addOverlay(
        function (container) {
          ...
          if (values.planned.length > 1000) {
            this.overloadedDataGanttNotification?.();
            this.overloadedDataGanttNotification = this.notification.add(
          ...
          // To do-> This line can be Updated in future if any other Module is using Graph Overlay
          if(myChart.$context){
            gantt.config.ks_overlay_error = true;
          }
          return canvas;
        }.bind(this)
      );
...
//       Check if parent task is available then set add parent otherwise set to false.
    ...
    ks_get_stage_data(stage_field, stage_data) {
    ...
    ks_project_task_data_update(
      ...
      var ks_gantt_datas = {
        ...
        ks_deadline_tooltip: each_project_task.deadline
          ? (parseDateTime(each_project_task.deadline,localization.dateTimeFormat)).toJSDate()
          : false,
...
//                        'ks_allow_parent_task': each_project_task.ks_allow_subtask,
      ...
      // Check if parent task is available then set add parent otherwise set to false.
      // if (!this.props.model.ksDataForGantt.KsAllTaskIds.includes(ks_gantt_datas.parent))
    ...
    ksConversionUtcToLocal(ksDate) {
      if (ksDate){
        const ks_date = ksDate.plus({minutes:ksDate.offset});
        return ks_date.toJSDate();
      }else{
        return false;
      }
      }
    ...
    _ksFullScreen(ev) {
      ...
      if (!gantt.config.ks_is_fullscreen) {
        ...
        gantt.config.ks_relative_fullscreen = true;
        ev.currentTarget.classList.remove("fa-expand");
        ev.currentTarget.classList.add("fa-compress");
        document.body.classList.add("ks_gantt_vertical");
        ...
        document.body.classList.remove("ks_gantt_vertical");
      }
    }
    ...
    _ksToggleOverlay(ev) {
      ...
      if (gantt.ext.overlay.isOverlayVisible(lineOverlay)) {
        ...
        // to inform user to refresh if myChart is not perfect
        if(gantt.config.ks_overlay_error){
            this.closeNotificationFn?.();
            this.closeNotificationFn = this.notification.add(
            "You need to refresh the page to use the Graph Overlay feature, once done kindly click on Graph Overlay again.",
            {
                title: _t("Warning"),
                type: 'warning',
                buttons: [
            ...
            gantt.ext.$overlay_area.style.display = "none"
            this.ks_disable_graph_overlay();
            gantt.config.ks_overlay_error = false
            return
        }
      ...
      //To-Do this is line to check if Graph Overlay is already active or not, Can be update in future
      if(gantt.config.readonly){
        gantt.ext.$overlay_area.style.display = "block"
      }
    }

    _ksToggleNoOfDays(ev) {
      if (!gantt.config.ks_no_of_days) {
    ...
    _ksToggleGrid(ev) {
      ...
      if(document.getElementById("ks_toggle_overlay")){
          this._ksDisplayGraphOverlay()
      }
    }

    _ksToggleResource(ev) {
      if (!gantt.config.ks_show_resource_panel) {
      ...
      Object.values(document.querySelectorAll(".ks-resource-mode")).map((item) => item.addEventListener("click",this._ksResourceMode.bind(this)));
    }
    ...
    ks_unmount_gantt(){
      gantt.clearAll();
      gantt.config.ks_attached_events.forEach((ks_event)=>{
    ...
    // To display Graph Overlay After disabling Grid and Resource Panel
    _ksDisplayGraphOverlay(){
        if (gantt.config.ks_show_resource_panel || gantt.config.ks_hide_grid_panel){
            document.getElementById("ks_toggle_overlay").disabled = true;
        }else{
            document.getElementById("ks_toggle_overlay").disabled = false
            gantt.init($(".ks_gantt_view_content").get(0));
            this.ks_gantt_overlay_canvas();
        }
    }
    ...
    ksGanttRenderer.template="ks_gantt_view_base.ks_gantt_content";
