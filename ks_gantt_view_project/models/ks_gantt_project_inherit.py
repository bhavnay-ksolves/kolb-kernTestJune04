                ...
                for i in range(0, len(ks_task.user_ids)):
                    if ks_task.date_deadline and ks_task.user_ids:
                        if ks_today_date < ks_task.date_deadline and (ks_task.date_deadline - ks_today_date).days in [7, 3,
                            ...
                            message_body = _(
                                "Hi %s, This mail is to remind you that task %s of project %s have only %s days for the deadline.") % \
                                           (
                                               ks_task.user_ids[i].name, ks_task.name, ks_task['project_id'].name,
    ...
    def _check_valid_ks_days_off_selection(self):
        for record in self:
            if len(record.ks_days_off_selection) == 7:
                raise UserError("Invalid value for Days selection. At least keep one working day")

    def write(self, vals):
        res = super(KsGanttViewProject, self).write(vals)
