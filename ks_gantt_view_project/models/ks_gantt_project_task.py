...
class KsProjectTask(models.Model):
    ...
    def ks_convert_day_names_to_integers(self, day_names):
        ...
        day_integers = [day_name_to_int.get(day_name) for day_name in day_names]
        return day_integers
    ...
    def create(self, values):
        ...
        if res.ks_task_duration and res.ks_enable_task_duration:
            ids = self.project_id.ks_days_off_selection.ids
            if ids:
                self.env.cr.execute('SELECT ks_day_name FROM ks_week_days WHERE id IN %s', (tuple(ids),))
                records = self.env.cr.dictfetchall()
                ks_day_names = [record['ks_day_name'] for record in records]
                day_integers = self.ks_convert_day_names_to_integers(ks_day_names)
                self.ks_end_datetime = self.ks_calculate_end_date(self.ks_task_duration, self.ks_start_datetime,
                                                                  day_integers)
            else:
    ...
    def ks_validate_constraint(self):
        ...
        for task in self:
            ...
            if task.ks_constraint_task_type == 'snet' and not task.ks_constraint_task_date <= task.ks_start_datetime:
            ...
            if task.ks_constraint_task_type == 'snlt' and not task.ks_constraint_task_date >= task.ks_start_datetime:
            ...
            if task.ks_constraint_task_type == 'fnet' and not task.ks_constraint_task_date <= task.ks_end_datetime:
            ...
            if task.ks_constraint_task_type == 'fnlt' and not task.ks_constraint_task_date >= task.ks_end_datetime:
            ...
            if task.ks_constraint_task_type == 'mso' and task.ks_constraint_task_date != task.ks_start_datetime:
            ...
            if task.ks_constraint_task_type == 'mfo' and task.ks_constraint_task_date != task.ks_end_datetime:
    ...
    def ks_fetch_timesheet_report(self, mail_template):
        ...
        result, format = report_template._render_qweb_pdf(report_template, [self.id])
        ...
        for rec in self:
            if rec.ks_start_datetime:
                ...
                if ids:
                    ...
                    ks_day_names = [record['ks_day_name'] for record in records]
                    day_integers = self.ks_convert_day_names_to_integers(ks_day_names)
                    rec.ks_end_datetime = self.ks_calculate_end_date(rec.ks_task_duration, rec.ks_start_datetime,
                                                                     day_integers)
                else:
                    rec.ks_end_datetime = rec.ks_start_datetime + timedelta(days=rec.ks_task_duration)
        ...
        for rec in self:
            ...
            if rec.user_ids and rec.user_ids.employee_id and rec.user_ids.employee_id.resource_calendar_id:
                ...
                for ks_avail_hours in ks_working_calendar.attendance_ids:
                    # Check for Sunday
                    if int(ks_avail_hours.dayofweek) == 6 and not resource_availability.get(0):
                        resource_availability[0] = []
                    elif not resource_availability.get(int(ks_avail_hours.dayofweek) + 1):
                    ...
                    while ks_temp_hours < ks_avail_hours.hour_to:
                        if int(ks_avail_hours.dayofweek) == 6:
